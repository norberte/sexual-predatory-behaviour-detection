...
print("First Level Classifier - SVM: 10 fold cross validation")
for(i in 1:10){
  ...
  # fit lda model
  libLinear <- LiblineaR(data = cvData_train, target = factor(cvLabels_train), type = 4)
  predictLibLinear <- predict(libLinear, as.matrix(cvData_test))
  
  # predicted on the whole data set
  cvFold_predictions[(((i-1)*10000) + 1):(i*10000)] <- predictLibLinear$predictions
  
  # calculate error rate
  errorRate[i] <- (nrow(cvData_test)-sum(diag(table(predictLibLinear$predictions, cvLabels_test))))/nrow(cvData_test)
