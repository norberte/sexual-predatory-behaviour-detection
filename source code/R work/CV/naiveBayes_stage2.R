...
for(i in 1:10){
  ...
  model <- naiveBayes(factor(cvLabels_train)~., data=cvData_train)
  prediction <- predict(model, cvData_test, type = "class")
  
  # predicted on the whole data set
  cvFold_predictions_svm[ind:(ind-1+length(testIndexes))] <- prediction
  ind <- ind + length(testIndexes)
  
  # calculate error rate
  errorRate_svm[i] <- (nrow(cvData_test)-sum(diag(table(cvLabels_test, prediction))))/nrow(cvData_test)
  
  # Precision for predator = TN / (TN+FN)
  # Recall for predator = TN / (TN+FP)
  confMatr <- table(prediction, cvLabels_test)
